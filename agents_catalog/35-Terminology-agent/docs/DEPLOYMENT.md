...
### TL;DR version
Here are the commands to deploy backend, OLS MCP server, and frontend:
```bash
# Step 1: Deploy backend infrastructure (creates Cognito)
cd infra-cdk
npm install
...
# Step 2: Deploy OLS MCP Server (uses Cognito from step 1)
...
### 4. Deploy OLS MCP Server (Ontology Lookup Service)

**IMPORTANT**: Deploy the OLS MCP Server **after** the backend stack. It requires Cognito credentials created in the previous step.

```bash
# From the agents_catalog/34-Terminology-agent directory
cd ..  # If you're still in infra-cdk
python deploy_ols_mcp_server.py --stack-name terminology-agent
```

This will:
- Clone the OLS MCP server from GitHub
- Patch it for AgentCore Runtime compatibility
- Deploy it as a separate AgentCore Runtime
- Configure M2M authentication with Cognito from the backend stack
- Store connection details in SSM parameters

**Note**: This step takes approximately 3-5 minutes. You can verify the deployment with:
```bash
...
### 6. Create a Cognito User (if necessary)
