...
Interactive chat interface and automated test suite for testing agent invocations. Automatically detects the agent pattern from `infra-cdk/config.yaml`.

**Modes:**

- **Remote** (default): Test deployed agent via Cognito authentication
- **Local** (`--local`): Test agent running on localhost:8080
- **Test Suite** (`--test-suite`): Run automated terminology tests

**Usage:**

```bash
# Remote interactive chat (prompts for credentials)
uv run test-scripts/test-agent.py

# Local interactive chat (auto-starts agent if not running)
...
When using `--test-suite`, runs 5 automated tests:
1. Simple Term Lookup (MONDO ID for diabetes)
2. Term with Hierarchy (parent/child terms for myocardial infarction)
3. Multi-Ontology Search (insulin in ChEBI and GO)
4. Term Standardization (heart attack, blood pressure)
5. Ontology Information (MONDO ontology description)
...
- Test Suite: Works with both local and remote modes
